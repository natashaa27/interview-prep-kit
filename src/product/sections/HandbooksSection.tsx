...
export const HandbooksSection: React.FC<{ handbooks: Handbook[] }> = ({ handbooks }) => (
  <div>
    <SectionHeader
      title="Handbooks"
      subtitle="Overview of all source materials — click 'Open PDF' to read the original, hosted directly on this site"
    />
    <div
...
const HandbookCard: React.FC<{ handbook: Handbook }> = ({ handbook: hb }) => {
  const pdfUrl = hb.pdfFile ? `${import.meta.env.BASE_URL}pdfs/${hb.pdfFile}` : null;
  ...
  return (
    <Card style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ fontSize: 11.5, color: hb.color, fontWeight: 600, marginBottom: 12 }}>
        {hb.source} &middot; {hb.pages} pages
      </div>
